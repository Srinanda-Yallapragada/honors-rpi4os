...
//
enum {
  PERIPHERAL_BASE = 0xFE000000,          // base for all the peripherals
  GPFSEL0 = PERIPHERAL_BASE + 0x200000,  // GPIO Function selector 0
  GPSET0 = PERIPHERAL_BASE + 0x20001C,   // GPIO  Pin Output Set 0
  GPCLR0 = PERIPHERAL_BASE + 0x200028,   // GPIO Pin Output Clear 0
  GPPUPPDN0 = PERIPHERAL_BASE + 0x2000E4 // GPIO Pull up Pull Down
};
...
void mmio_write(long reg,
                unsigned long val) { // write to main mem long(32 bit) was
                                     // unsigned int before, but i think it
                                     // makes sense to switch to u long
  *(volatile unsigned int *)reg = val;
}
unsigned long mmio_read(long reg) {
  return *(volatile unsigned int *)reg;
} // read from a long mem addr
...
// UART
// see https://datasheets.raspberrypi.com/bcm2835/bcm2835-peripherals.pdf page 8
// for a table with all the registers
enum {
