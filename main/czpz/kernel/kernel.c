...
void timer_init() {
  ...
  timer1_val +=
      timer1_int; // sets the value of the first timer interrupt to CLOCKHZ
...
void handle_timer_1() {
  ...
  REGS_TIMER->control_status |=
      SYS_TIMER_IRQ_1; // update control status register to clear the match
                       // deteched status bit
  num_timer_handle_timer_called += 1;

  char *out_to_uart = "";
  getDecStr(out_to_uart, 3, num_timer_handle_timer_called); // gets the decimal string of num_timer_handle_timer_called
  uart_writeText(out_to_uart); // write num_timer_handle_timer_called to uart
  uart_writeText("handle_timer_1 went off! \n");
}

void main(void) {
  num_timer_handle_timer_called = 0; // resetting to 0
