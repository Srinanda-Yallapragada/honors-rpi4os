...
enum { // gpio function select
  GPIO_MAX_PIN = 53,
  GPIO_FUNCTION_ALT5 = 2,
};

enum { // gpio pull up/down
  Pull_None = 0,
};

void mmio_write(long reg, unsigned int val) {
  *(volatile unsigned int *)reg = val;
} // volatile write to address
unsigned int mmio_read(long reg) {
  return *(volatile unsigned int *)reg;
} // volatile read from address
...
unsigned int gpio_call(unsigned int pin_number, unsigned int value,
  ...
  unsigned int num_fields =
      32 / field_size; // number of fields that can fit in a 32-bit register
  unsigned int reg = base + ((pin_number / num_fields) *
                             4); // get the register address to be accessed
  unsigned int shift =
      (pin_number % num_fields) * field_size; // get the shift amount
...
enum {
  // miniUART register addresses
  AUX_BASE =
      PERIPHERAL_BASE + 0x215000,   // Base address of the auxiliary peripherals
  AUX_ENABLES = AUX_BASE + 4,       // Auxiliary enables register
  AUX_MU_IO_REG = AUX_BASE + 64,    //  miniUART I/O data register
  AUX_MU_IER_REG = AUX_BASE + 68,   //  miniUART interrupt enable register
  AUX_MU_IIR_REG = AUX_BASE + 72,   //  miniUART interrupt identify register
  AUX_MU_LCR_REG = AUX_BASE + 76,   //  miniUART line control register
  AUX_MU_MCR_REG = AUX_BASE + 80,   //  miniUART modem control register
  AUX_MU_LSR_REG = AUX_BASE + 84,   //  miniUART line status register
  AUX_MU_CNTL_REG = AUX_BASE + 96,  //  miniUART control register
  AUX_MU_BAUD_REG = AUX_BASE + 104, //  miniUART baudrate register
...
// checking if transmitter idle which implies ready to write
...
void uart_writeText(char *buffer) {
  while (*buffer) // check end of string, write out entire text byte by byte
  {
    ...
    uart_writeByteBlockingActual(
        *buffer++); // writing out the char and incrementing the pointer
  }
}
